...
import { fetchJsonSetting, saveJsonSetting, DEFAULT_SALES_SETTINGS, SalesSettings } from '@/lib/settings';
...
export function SalesSettingsTab({
  ...
  const loadSettings = async () => {
    setLoading(true);
    const data = await fetchJsonSetting<SalesSettings>('sales_settings', DEFAULT_SALES_SETTINGS);
    setSettings(data);
    setDbSettings(data);
    setLoading(false);
  };
  ...
  const handleSave = async (reason?: string): Promise<boolean> => {
    const err = await saveJsonSetting('sales_settings', settings);
    if (reason && !err) {
